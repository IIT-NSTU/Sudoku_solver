public class Match {
    ...
    public void exist(Board1 board1, String word, int row, int col) {
        for (int i = 0; i < row; i++) {
            for (int j = 0; j < col; j++) {
                if (Character.toLowerCase(board1.get_data1(i, j) )== word.charAt(0) && dfs(board1, i, j, 0, word)) {
    ...
    private boolean dfs(Board1 board1, int i, int j, int count, String word) {
        ...
        if (Character.toLowerCase(board1.get_data1(i, j)) != word.charAt(count)) {
            return false;
        }

        char temp = Character.toLowerCase(board1.get_data1(i, j));
